...
Rules are enforced when the transaction is created, so they cover a [send](/transactions/sends), a [swap](/transactions/swaps), or bridge that pays an external recipient. A send that no rule allows is rejected. In the send form, the token and network pickers offer only what every selected recipient accepts, and picking a recipient that rejects the selected token clears that token.
