...
An invoice requests a single amount of a single token into one of your accounts (there are no line items), payable in crypto or, if enabled, [by bank transfer](#pay-by-bank). Invoicing is free: no fee on issuing or on crypto payments. Only [Owners](/teams/permissions) create and manage invoices; any member can view them.
...
Pay-by-bank lets the invoicee pay in fiat from their bank account. It requires your team to have [banking](/banking) set up with a virtual account in the matching currency, and the requested token to be:
...
Bank payments settle to the account and network configured in your bank settings, even when the invoice specifies a different account or network.
