...
A Splits account can be reconstructed at the same address on an unsupported EVM network, then used to bridge stranded assets to a supported network. This is an exceptional recovery procedure, not an app flow.

Prerequisites: an RPC for the unsupported network, the account's original creation parameters, and access to enough [recovery signers](/teams/recovery#recovery-signers) to meet the recovery threshold.
