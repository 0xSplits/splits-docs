...
A dedicated [account](/accounts) per project, experiment, or revenue source. If the project flops, close the account; if it succeeds, it can become the start of a new workspace. Separate accounts per asset type (stables, investments, NFTs) also keep books clean while still exporting as [one CSV](/accounting).
...
Two members on one workspace, registered with different emails (e.g. Gmail and Proton), each with its own passkeys. Moving large amounts then requires access to both inboxes.
