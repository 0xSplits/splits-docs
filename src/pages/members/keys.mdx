...
# Signing keys [The passkeys and EOAs a member signs with, across every workspace]

A member's signing keys are what sign transactions: **passkeys** saved in the app, or **EOAs** registered via the CLI. Keys belong to the [member](/members), not to any workspace, so they work across every workspace the member is part of. The private key stays with the member; **Splits never has access to private keys.**
...
An EOA (externally owned account, a standard Ethereum keypair) can be [registered via the CLI](/introduction/agents#sign-locally-with-an-eoa) and added as a signer, letting agents and servers sign headlessly. A workspace's [recovery signers](/workspaces/recovery#recovery-signers) are also EOAs.
