...
Splits never has custody of a team's assets, so there must be a way to regain them without passkeys. Passkeys are phishing-resistant because they're bound to the issuing domain. If splits.org went offline, or a passkey manager were compromised or lost, the passkeys would become unusable. Wallet-based recovery signers are ordinary Ethereum keys with no such dependency: they work as long as the chain does.
...
Confirm a wallet-based recovery signer's private key is still accessible without moving funds: Splits has the key sign a test message. In *Settings > Recovery*, open the signer's three-dot menu → *Verify signer*; this requires connecting the wallet. Each verified signer shows when it was last verified.
...
- [Verify wallet-based recovery signers](#verifying-recovery-signers) periodically; each shows when it was last verified.
