...
Signing authority is separate from [membership](/members) and [roles](/workspaces/roles). The two often overlap 1:1, but are distinct:

| | Workspace member | Signer |
| --- | --- | --- |
| Stored | Offchain | Onchain (account's signer set) |
| Scoped to | A workspace | An account |
| What it is | A person's association with a workspace | A signing key belonging to a member |
| Grants | Visibility + [role](/workspaces/roles) capabilities | Authority to approve transactions, counted against the account's threshold |
...
1. **Every signer on a workspace's accounts must be a member of that workspace.**
2. **Neither membership nor any role ever makes someone a signer.** Adding a signer is a separate, onchain action.

## Recovery signers

The [Root account](/accounts#root)'s signers are the workspace's recovery signers: EOAs chosen at workspace setup that can reset every other account's signers. See [Recovery](/workspaces/recovery#recovery-signers).
