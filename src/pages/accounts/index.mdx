...
Ownership carries the powers signers don't have: it is how a [signer reset](/accounts/editing#resetting-signers) and [recovery](/teams/recovery) work, and it bounds what a compromised signing key can reach. The [security model](/accounts/security) covers what the contract enforces. The Root has no external owner and owns itself: owner actions on it fall to its own signers, the recovery signers, at the recovery threshold.
