...
| Treasury | Root | [Recovery signers](/workspaces/recovery#recovery-signers), connected as wallets, at the recovery threshold |
...
If every passkey on the workspace is lost, reset the Treasury first (recovery wallets), then use the restored Treasury to reset any sub-accounts. See [Recovery](/workspaces/recovery) for the full lost-passkey scenario.
