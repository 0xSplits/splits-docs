...
- **Native on Base**: no bridges, no wrapped assets.

The APY shown in the app is the wrapper's net APY, reported by Morpho, after the Splits fee.

### Fees

Splits charges a **20% performance fee** on the yield the vault earns and **no management fee**. The fee is taken inside the vault: as interest accrues, the wrapper issues shares worth 20% of the new yield to Splits' fee account, and the remaining 80% raises the share price for every depositor. Changing the fee or its recipient is an onchain action with a 3-day timelock; Morpho caps a wrapper's performance fee at 50%.
...
Because Earn is a deposit into (and withdrawal from) a lending position, most accounting tools handle it without special treatment. The performance fee is deducted inside the vault and does not appear as a separate transaction.
