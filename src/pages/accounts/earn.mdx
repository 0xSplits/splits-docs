...
Using Earn does not require you to manually account for vault shares or calculate the yield yourself. The [Accounting page](/accounting) handles the position directly: in the *Tokens* view, it reports the position in underlying USDC, and records net yield under *Change from flow* rather than as a token-price gain.

Deposits and withdrawals remain in the transaction history and CSV exports. The displayed yield is net of the performance fee, which is deducted inside the vault and does not appear as a separate transaction.

Splits provides these records and calculations, but it does not determine how they should be classified for your specific tax or accounting situation. Consult your accountant if you need guidance on that classification.
